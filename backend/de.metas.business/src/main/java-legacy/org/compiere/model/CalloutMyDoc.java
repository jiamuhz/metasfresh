...
public class CalloutMyDoc extends CalloutEngine
{
	...
	/**
	 * C_MyDoc.C_DocTypeTarget_ID changed: - InvoiceRuld/DeliveryRule/PaymentRule - temporary Document Context: - DocSubType - HasCharges - (re-sets Business Partner info of required)
	 */
	public String docType(final ICalloutField calloutField)
	{
		final I_C_MyDoc myDoc = calloutField.getModel(I_C_MyDoc.class);

		final I_C_MyDoc oldMyDoc = InterfaceWrapperHelper.createOld(myDoc, I_C_MyDoc.class);

		final DocTypeId newDocTypeId = DocTypeId.ofRepoIdOrNull(myDoc.getC_DocTypeTarget_ID());
		...
		final IDocumentNoInfo documentNoInfo = Services.get(IDocumentNoBuilderFactory.class)
				...
				.setOldDocType_ID(oldMyDoc.getC_DocTypeTarget_ID())
				.setOldDocumentNo(myDoc.getDocumentNo())
				.setDocumentModel(myDoc)
				.buildOrNull();
		...
		{
			myDoc.setDocumentNo(documentNoInfo.getDocumentNo());
		}
	...
	private interface DropShipPartnerAware
	{
		...
		void setDropShip_Location(I_C_BPartner_Location DropShip_Location);

		void setDropShip_User_ID(int DropShip_User_ID);

		void setDropShip_User(I_AD_User DropShip_User);
	}
