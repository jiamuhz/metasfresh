...
class DocumentSalesRepDescriptorFactoryTest_Order
{
	...
	void forDocumentRecord_C_Order(
			boolean soTrx,
			boolean createSalesRep,
			boolean orderSalesPartnerRequired,
			String orderSalesPartnerCode)
	{
		final BPartnerTestRecords testRecords = BPartnerTestRecords.builder().createSalesRep(createSalesRep).build();

		final OrderTestRecord orderTestRecord = OrderTestRecord.builder()
				...
				.salesPartnerRequired(orderSalesPartnerRequired)
				.salesPartnerCode(orderSalesPartnerCode)
				.build();

		final DocumentSalesRepDescriptor result = documentSalesRepDescriptorFactory.forDocumentRecord(orderTestRecord.getOrderRecord());
		orderTestRecord.assertMatches(result);
	}
}
