...
public class ImportHelperTest
{
	...
	public void test_setReplicationCtx() throws Exception
	{
		...
		Env.setContextItem(initialCtx, Env.CTXNAME_AD_Client_ID, 12345);

		final Properties ctx = Env.deriveCtx(initialCtx);
		Assert.assertEquals(Env.getContextItemAsInt(ctx, Env.CTXNAME_AD_Client_ID), 12345);

		setReplicationCtx(ctx, Env.CTXNAME_AD_Client_ID, 1, false); // overwrite = false
		Assert.assertEquals(Env.getContextItemAsInt(ctx, Env.CTXNAME_AD_Client_ID), 1);
		...
		Assert.assertEquals(Env.getContextItemAsInt(ctx, Env.CTXNAME_AD_Client_ID), 0);
	}
