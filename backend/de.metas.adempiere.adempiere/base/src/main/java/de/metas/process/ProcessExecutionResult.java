...
public class ProcessExecutionResult
{
	...
	public enum ViewOpenTarget
	{
		/**
		 * 作为子视图打开
		 * 新视图不会占据整个屏幕，而是作为当前视图的一部分（如一个子页签或嵌入的区域）被包含和显示。
		 * 这是典型的主从表（Master-Detail）操作。比如，在销售订单列表中点击某一行，下方展开显示该订单的行信息。
		 */
		IncludedView,

		/**
		 * 作为模态弹窗打开
		 * 新视图会以弹窗（对话框）的形式叠加在当前视图之上。它会暂时阻止用户与后台页面交互，直到用户完成弹窗内的操作（如填写表单）并关闭它。
		 * 常用于需要用户立即关注或临时处理的场景，例如点击某个按钮弹出“新建订单”窗口。
		 */
		ModalOverlay,

		/**
		 * 在浏览器的新标签页打开
		 * 新视图会直接在一个全新的浏览器标签页中加载。
		 * 这种方式会将用户完全带入一个独立的上下文中，适合查看详情或执行长时间操作而不希望丢失当前页面状态。
		 * 例如，在报表列表中点击“运行报表”，报表结果在一个新标签页中打开。
		 */
		NewBrowserTab
	}
