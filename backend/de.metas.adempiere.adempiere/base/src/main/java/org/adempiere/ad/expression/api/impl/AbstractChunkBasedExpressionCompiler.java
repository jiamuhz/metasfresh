...
public abstract class AbstractChunkBasedExpressionCompiler<V, ET extends IExpression<V>> implements IExpressionCompiler<V, ET>
{
	...
	public final ET compile(final ExpressionContext context, final String expressionStr)
	{
		...
		while (i != -1)
		{
			...
			inStr = inStr.substring(i + PARAMETER_TAG_LENGTH);    // from first marker
			final int j = inStr.indexOf(PARAMETER_TAG);                     // next parameter marker
			if (j < 0)
			{
				// no second tag
				throw new ExpressionCompileException("Missing closing tag " + PARAMETER_TAG + " in currentSubstring")
						.appendParametersToMessage()
						.setParameter("currentSubstring", inStr)
						.setParameter("expressionString", expressionStr);
			}
			else if (j == 0)
			{
				// Double marker (e.g. @@)
			...
			inStr = inStr.substring(j + PARAMETER_TAG_LENGTH);    // from second @ to end
